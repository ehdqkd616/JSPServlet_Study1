...
<table>
<Tr>
<td>사원번호</td><td><input type=text name=empId
value="${emp.employeId}" ${empty emp ? "" : readonly}></td>
</Tr>
<tr>
...
${emp.jobId eq job.jobId ? "selected" : ""}>
${job.jobTitle}</option>
</c:forEach>
</select></td></tr>
<tr>
...
<tr>
<td>보너스율</td><td><input type=number min=0 max=1 step=0.05
name=commissionPct value ="${emp.commissionPct}"></td>
</tr>
<tr>
<td>매니저</td><td><select name=managerId>
<c:forEach var=man items="${manList}">
<option value="${man.employeeId}"
${man.employeeId eq emp.managerId ? "selected" : ""}>
...
${dept.departmentId eq emp.departmentId ? "selected" : ""}>
